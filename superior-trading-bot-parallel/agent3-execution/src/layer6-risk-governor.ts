 ...
 *
 * Phase 1: MONITORING ONLY - veto events are logged, not enforced.
 */
...
class CircuitBreakerEngine {
  ...
  /**
   * Update a circuit breaker's current value and check if it should trip.
   * Returns a RiskVeto if the breaker trips, null otherwise.
   */
...
export class RiskGovernor extends EventEmitter {
  ...
  constructor(config: {
    ...
    super();
    this.drawdownTracker = new DrawdownTracker(config.initialEquity || 100000);
    this.limits = {
      maxDailyDrawdown: 0.05,          // 5%
      maxWeeklyDrawdown: 0.10,         // 10%
      maxSingleTradeRisk: 0.02,        // 2%
      maxOpenPositions: 10,
      maxCorrelationExposure: 0.7,
      maxSectorExposure: 0.3,
      emergencyShutdownThreshold: 0.08, // 8%
      ...config.limits,
    };
  ...
  private initializeCircuitBreakers(): void {
    ...
    this.circuitBreakers.add({
      id: 'portfolio-daily-dd',
      level: 'portfolio',
      targetId: 'portfolio',
      state: 'closed',
      tripThreshold: this.limits.maxDailyDrawdown,
      resetThreshold: this.limits.maxDailyDrawdown * 0.5,
      currentValue: 0,
      tripCount: 0,
      lastTripped: null,
    ...
    // Portfolio-level: Weekly drawdown
    this.circuitBreakers.add({
      id: 'portfolio-weekly-dd',
      level: 'portfolio',
      targetId: 'portfolio',
      state: 'closed',
      tripThreshold: this.limits.maxWeeklyDrawdown,
      resetThreshold: this.limits.maxWeeklyDrawdown * 0.5,
      currentValue: 0,
      tripCount: 0,
      lastTripped: null,
    ...
    // Portfolio-level: Emergency shutdown
    this.circuitBreakers.add({
      id: 'portfolio-emergency',
      level: 'portfolio',
      targetId: 'portfolio',
      state: 'closed',
      tripThreshold: this.limits.emergencyShutdownThreshold,
      resetThreshold: this.limits.emergencyShutdownThreshold * 0.3,
      currentValue: 0,
      tripCount: 0,
      lastTripped: null,
    ...
    // Position-level: Max open positions
    this.circuitBreakers.add({
      id: 'position-count',
      level: 'position',
      targetId: 'all-positions',
      state: 'closed',
      tripThreshold: this.limits.maxOpenPositions,
      resetThreshold: this.limits.maxOpenPositions * 0.7,
      currentValue: 0,
      tripCount: 0,
      lastTripped: null,
    ...
    // Strategy-level: Correlation risk
    this.circuitBreakers.add({
      id: 'strategy-correlation',
      level: 'strategy',
      targetId: 'all-strategies',
      state: 'closed',
      tripThreshold: this.limits.maxCorrelationExposure,
      resetThreshold: this.limits.maxCorrelationExposure * 0.6,
      currentValue: 0,
      tripCount: 0,
      lastTripped: null,
    ...
    // Strategy-level: Single trade risk
    this.circuitBreakers.add({
      id: 'strategy-single-trade',
      level: 'strategy',
      targetId: 'single-trade',
      state: 'closed',
      tripThreshold: this.limits.maxSingleTradeRisk,
      resetThreshold: this.limits.maxSingleTradeRisk * 0.5,
      currentValue: 0,
      tripCount: 0,
      lastTripped: null,
  ...
  private runRiskChecks(): void {
    ...
    const dailyDD = this.drawdownTracker.getDailyDrawdown();
    const v1 = this.circuitBreakers.update('portfolio-daily-dd', dailyDD);
    if (v1) vetoes.push(v1);

    // Check weekly drawdown
    const weeklyDD = this.drawdownTracker.getWeeklyDrawdown();
    const v2 = this.circuitBreakers.update('portfolio-weekly-dd', weeklyDD);
    if (v2) vetoes.push(v2);

    // Check drawdown from peak (emergency)
    const peakDD = this.drawdownTracker.getDrawdownFromPeak();
    const v3 = this.circuitBreakers.update('portfolio-emergency', peakDD);
  ...
  /**
   * Evaluate a proposed trade. Returns null if approved,
   * or a RiskVeto if the trade should be blocked.
   */
  evaluateTrade(proposal: {
    ...
    // Check if any portfolio-level breakers are open
    ...
    return null; // Trade approved
  }

  // ===================== STATE UPDATES =====================

  updateEquity(equity: number): void {
    this.drawdownTracker.updateEquity(equity);
  }
  ...
  setTotalExposure(exposure: number): void {
    this.totalExposure = exposure;
  }

  setCorrelationRisk(risk: number): void {
