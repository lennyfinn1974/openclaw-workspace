...
class KanbanBackendServer {
  ...
  setupMiddleware() {
    ...
    // Serve static files from public directory (migration data, etc.)
  ...
  setupRoutes() {
    ...
    // API to get combined data (live only by default, add ?include=migration for historical)
    this.app.get('/api/data/combined', async (req, res) => {
      try {
        const result = {
          boards: [...this.liveTickets.boards],
          columns: [...this.liveTickets.columns],
          tasks: [...this.liveTickets.tasks],
          agents: [...this.liveTickets.agents],
          activities: [...this.liveTickets.activities]
        };

        // Only include migration data if explicitly requested
        if (req.query.include === 'migration') {
          const migrationData = JSON.parse(await fs.readFile(this.migrationDataPath, 'utf8'));
          result.boards.push(...migrationData.boards);
          result.columns.push(...migrationData.columns);
          result.tasks.push(...migrationData.tasks);
          result.agents.push(...migrationData.agents);
          result.activities.push(...migrationData.activities);
        }
